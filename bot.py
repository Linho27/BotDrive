...
DISCORD_USER_ID = int(os.getenv("DISCORD_USER_ID"))  # ID do utilizador de Discord
...
async def send_drive_link_for_game(interaction, jogo):
    try:
        ...
        if files:
            ...
            # Se não encontrar o ficheiro, perguntar se o utilizador quer o pedir
            await interaction.response.edit_message(content=f"❌ Nenhum ficheiro encontrado para `{jogo['name']}`. Gostarias de pedir o ficheiro?", embed=None, view=PedidoFicheiroView(jogo))
    except Exception as e:
        await interaction.response.edit_message(content=f"❌ Erro: {e}", embed=None, view=None)


# ================================
# 🔘 Pedido de Ficheiro - Botões
# ================================
class PedidoFicheiroView(View):
    def __init__(self, jogo):
        super().__init__(timeout=60)  # Tempo para interação (1 minuto)
        self.jogo = jogo

    @discord.ui.button(label="Sim, pedir o ficheiro", style=discord.ButtonStyle.green)
    async def pedir_ficheiro(self, button: Button, interaction):
        # Enviar mensagem direta ao utilizador do Discord
        user = client.get_user(DISCORD_USER_ID)
        if user:
            await user.send(f"🔔 O utilizador {interaction.user} pediu o ficheiro para o jogo `{self.jogo['name']}`. Aguardar resposta.")
            await interaction.response.edit_message(content=f"Pedido de ficheiro para `{self.jogo['name']}` enviado! Aguardar resposta.", embed=None, view=None)
        else:
            await interaction.response.edit_message(content="❌ Não foi possível enviar a mensagem ao utilizador. Tente novamente mais tarde.", embed=None, view=None)
        
    @discord.ui.button(label="Não, obrigado", style=discord.ButtonStyle.red)
    async def nao_pedir(self, button: Button, interaction):
        await interaction.response.edit_message(content=f"Não será feito pedido para `{self.jogo['name']}`.", embed=None, view=None)


# ================================
# 📁 Comando ficheiro - Modificado
# ================================
